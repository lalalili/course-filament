...
1. 把 `composer.json` 的約束改成 `^1.0`：
   ...
   ```diff
   -"lalalili/course-filament": "^0.x"
   +"lalalili/course-filament": "^1.0"
   ```

2. 確認 `repositories` 是 `vcs` 而非指向本機 `packages/` 的 `path`：

   ```json
   { "type": "vcs", "url": "https://github.com/lalalili/course-filament.git" }
   ```

3. 更新並清快取：

   ```bash
   composer update lalalili/course-filament
